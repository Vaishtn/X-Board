...
let createAccordion = (title, id) => {
  return `<div class="accordion-item" id="card${id}">
    ...
    <div id="collapse${id}" class=" collapse " aria-labelledby="heading${id}" data-bs-parent="#accordionId"></div>`;
};

//createCauroselOuter
let createCauroselOuter = (id, innerId) => {
...
let addContent = async () => {
  for (let i = 0; i < magazines.length; i++) {
    ...
    let carousel = createCauroselOuter(carouselId, carouselInnerId);
...
// let items = data["items"];
    ...
    //   const carouselItem = createCauroselInner(carouselItemId, index === 0);
